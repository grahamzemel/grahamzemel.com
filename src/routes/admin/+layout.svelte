<script>
  ...
  /** @type {{ settings?: { autoSendTestOnSubscribe?: boolean } } | null} */
...
<div
  class="fixed inset-0 overflow-y-auto z-50 transition-colors duration-200 admin-shell
  {dark ? 'dark-mode bg-gray-950 text-gray-100' : 'bg-[#fafafa] text-gray-900'}"
>
  <!-- Top nav -->
  <header
    class="sticky top-0 z-10 border-b transition-colors duration-200
    {dark ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-200'}"
  >
    <div
      class="max-w-6xl mx-auto px-4 sm:px-6 flex items-center justify-between h-14"
    >
      <div class="flex items-center gap-8">
        <a
          href="/admin"
          class="text-sm font-semibold tracking-tight {dark
            ? 'text-gray-100'
            : 'text-gray-900'}">Income Engine</a
        >
        <!-- Desktop nav -->
      ...
      <div class="flex items-center gap-3">
        ...
        {#if pushSubscribed}
          <span
            class="hidden sm:inline text-[10px] text-emerald-500"
            title="Push notifications active">notifications on</span
          >
        {/if}
        <button
          on:click={logout}
          class="hidden md:inline text-xs transition {dark
        ...
        >
          {#if menuOpen}
            <svg
              class="w-5 h-5 {dark ? 'text-gray-300' : 'text-gray-600'}"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              stroke-width="2"
    ...
    {#if menuOpen}
      <nav
        class="md:hidden border-t px-4 pb-3 pt-1
      ...
      >
        ...
        {/each}
        <div
          class="border-t mt-2 pt-2 flex items-center justify-between px-3
...
<style global>
  ...
  .dark-mode .border-emerald-200 {
    border-color: rgb(6 95 70) !important;
  }
  ...
  .dark-mode .animate-pulse .bg-gray-200 {
    background-color: rgb(55 65 81) !important;
  }
</style>
