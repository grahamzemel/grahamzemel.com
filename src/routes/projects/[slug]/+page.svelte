...
<div class="pt-12 flex justify-between">
  ...
  <ButtonLink size="small" aria-label={`${data.project.title}'s source code`}  href={`${data.project.sourceCode}`}>
    Source Code
  </ButtonLink>
  {/if}
  {#if data.project.liveDemo}
  <ButtonLink size="small" aria-label={`${data.project.title}'s live release`} href={data.project.liveDemo}>
    View Live
  </ButtonLink>
  {/if}
</div>
